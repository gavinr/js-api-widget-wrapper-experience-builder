...
import { isPercentage, getBuilderThemeVariables } from 'jimu-layouts/common';
import { BorderSetting, BackgroundSetting, InputUnit } from 'jimu-ui/style-setting-components';
import { Switch, Select, UnitTypes, LinearUnit, Icon, NumericInput } from 'jimu-ui';
...
export default class Setting extends React.PureComponent<Props> {
  ...
  getStyle(builderTheme: IMThemeVariables) {
    return css`
      .toggle-btn-style {
        background: ${builderTheme.colors.white};
        border: 1px solid ${builderTheme.colors.palette.light[500]};
        width: 110px;
        height: 30px;
        cursor: pointer;

        &.selected {
          border: 1px solid ${builderTheme.colors.palette.primary[700]};
        }
      }
    `;
  }
  ...
  getDocksideStyle(builderTheme: IMThemeVariables) {
    return css`
      display: flex;
      ...
      cursor: pointer;
      background: ${builderTheme.colors.white};
      outline: 1px solid ${builderTheme.colors.palette.light[500]};
  ...
  render() {
    ...
    return (
      <div className="hsidebar-layout-setting" css={this.getStyle(builderTheme)}>
        <SettingSection title={formatMessage('panel')}>
          <SettingRow label={formatMessage('dockSide')} flow="wrap">
            <div className="d-flex justify-content-between w-100">
              <div css={this.getDocksideStyle(builderTheme)}
          ...
          <SettingRow label={formatMessage('size')}>
            <InputUnit units={availableUnits} value={parsePositionProp(config.size)} style={inputStyle} onChange={this.updateSize} />
          </SettingRow>
